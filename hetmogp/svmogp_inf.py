...
class SVMOGPInf(LatentFunctionInference):
    ...
    def inference(self, q_u_means, q_u_chols, X, Y, Z, kern_list, likelihood, B_list, Y_metadata, KL_scale=1.0,
        ...
        # inference for rest of cases
            ...
            posteriors = []
            for q in range(Q):
                (dL_dmu_q, dL_dL_q, posterior_q, dL_dKqq, dL_dKdq, dL_dKdiag_q) = self.calculate_gradients(q_U=q_U,
                                                                                                           p_U=p_U,
                                                                                                           q_F=q_F,
                                                                                                           VE_dm=VE_dm,
                                                                                                           VE_dv=VE_dv,
            ...
            gradients = {'dL_dmu_u': dL_dmu_u, 'dL_dL_u': dL_dL_u, 'dL_dKmm': dL_dKmm, 'dL_dKmn': dL_dKmn,
                         'dL_dKdiag': dL_dKdiag}
    ...
    def calculate_gradients(self, q_U, p_U, q_F, VE_dm, VE_dv, Ntask, M, Q, D, f_index, d_index, q):
        ...
        # Posterior
        posterior_q = Posterior(mean=m_u[:, q, None], cov=S_u[q, :, :], K=Kuu[q, :, :],
                                prior_mean=np.zeros(m_u[:, q, None].shape))
